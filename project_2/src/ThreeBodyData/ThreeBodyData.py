...
class ThreeBodyData:
    ...
    def create_high_dimensional_dataset(
        self, num_modes=6, poly_order=128, domain=[-1, 1]
    ):
        """
        ...
        if self.solution is None or not self.solution.success:
            raise ValueError(
                "The ODE solution must be successfully calculated before creating the high-dimensional dataset."
            )
        ...
        legendre_polys = np.polynomial.legendre.legval(
            grid_points, np.identity(num_modes)
        )
    ...
    def generate_datasets(
        ...
        initial_conditions = self.sample_initial_conditions(
            num_initial_conditions, position_bounds, velocity_bounds
        )
        ...
        for i in tqdm(range(num_initial_conditions), desc="Generating datasets"):
            ...
            if plot:  # For testing
                ...
                self.plot_animation(
                    positions=positions,
                    title=f"Animated Trajectories of Alpha Centauri System Bodies {i}",
                )
            ...
            if solution is not None and solution.success:
                ...
                logging.warning(
                    f"ODE solver did not converge for initial condition set {i}. Skipping this dataset."
                )

        if not data_list:  # Check if the list is empty
            raise RuntimeError(
                "No valid solutions were generated. Check the ODE solver and initial conditions."
            )
    ...
    def sample_initial_conditions(self, num_samples, position_bounds, velocity_bounds):
        ...
        for i in range(num_samples):
            ...
            v1_0 = [
                np.random.uniform(*velocity_bounds["u"]),
            ...
            initial_conditions[i, :] = np.concatenate(
                (r1_0, r2_0, r3_0, v1_0, v2_0, v3_0)
            )
    ...
    def plot_animation(
        ...
        lines = [
            ax.plot([], [], [], "-", lw=2, label=f"Body {i+1}")[0] for i in range(3)
        ]
...
>>>>>>> aaa9c9470c09a0728acadadbbcc824cc399dfe0b
            ...
            self.plot_animation(
                positions=positions,
                save_as_gif=False,
                filename=f"ThreeBodyDataset_{dataset}_random_simulation.gif",
                title=f"Random {dataset.capitalize()} Simulation",
            )

            print(
                f"Animation for a random {dataset} simulation saved as ThreeBodyDataset_{dataset}_random_simulation.gif"
            )
...
if __name__ == "__main__":
    ...
    num_initial_conditions = (
        10  # Total number of different initial conditions aka simulations
    )
