...
def get_lorenz_train_data(n_ics, noise_strength=0):
    ...
    x, dx = generate_lorenz_train_data(ics, t, input_dim, noise_strength)
...
def get_lorenz_test_data(n_ics, noise_strength=0):
    ...
    # Generate Lorenz data with the specified initial conditions
    data = generate_lorenz_test_data(
...
def generate_lorenz_train_data(
    ics, t, n_points, linear=True, normalization=None, sigma=10, beta=8 / 3, rho=28
):
    """
    Generate high-dimensional Lorenz data set.
...
if __name__ == "__main__":
    ...
    training_data = get_lorenz_train_data(1)
    print("Training Data Keys:", training_data.keys())
    print(f"x shape: {training_data['x'].shape}, dx shape: {
          training_data['dx'].shape}")

    # Test the get_lorenz_test_data function
    test_data = get_lorenz_test_data(1)
    print("Test Data Keys:", test_data.keys())
    print(f"x shape: {test_data['x'].shape}, dx shape: {
          test_data['dx'].shape}")
    print(f"z shape: {test_data['z'].shape}, dz shape: {
          test_data['dz'].shape}")
    print("SINDy Coefficients:", test_data['sindy_coefficients'])
